...
int Vehicle::next_lane(vector<vector<double>> sensor_fusion, int current_lane, double car_s, double end_path_s, int prev_size)
{
  ...
  // checking which lanes are not available
  if (lane==0)
    left_is_free = false;
  else if (lane==2)
    right_is_free = false;

  // find ref_v to use, search through sensor fussion
  for (int i = 0; i < sensor_fusion.size(); i++)
  {
    ...
    // the velocity of the car in current lane
    ...
    // validating that we will not crash with the car in front of us
    if ((car_dist > 0) && (car_dist < 30))
    {
      // if the observed car is on the left side
      ...
      // if the observed car is on the right side
      else if (lane_obsvd == (lane+1))
      {
    ...
    // if the observed car is in the current lane:
    ...
    // To make the lane change more smoothly
